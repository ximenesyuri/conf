...
hi def link shimShebang				Comment
...
hi def link shimVarBraces			Delimiter
...
hi def link shimBuiltinFunction		Type
...
" For WS
" ===========
syntax keyword shIntuiImport import 
syntax keyword shIntuiCoreFunction data help check log editor
syntax keyword shIntuiImportOptions from in
syntax match shIntuiCoreFunctionOptions "\s\<ws\>\+\<warn\>\+\<info\>\s"
hi! def link shIntuiImport SpecialKey
hi! def link shIntuiImportOptions Constant
hi! def link shIntuiCoreFunction SpecialKey
hi! def link shIntuiCoreFunctionOptions SpecialKey
syn match shFunctionName /\(WS\|MOD\|INT\)::[A-Za-z:]*()/
syntax cluster shCommandSubList add=shIntuiCoreFunction,shIntuiImportOptions,shIntuiCoreFunctionOptions
